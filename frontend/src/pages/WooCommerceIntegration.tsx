  ...
  return (
    <div className={`relative ${className}`} ref={ref}>
      <button
        ...
        className="w-full flex items-center justify-between px-3 py-2.5 border border-border rounded-lg text-sm bg-background text-foreground hover:bg-muted transition-colors text-left"
      >
        {selected ? (
      ...
      {open && (
        <div className="absolute left-0 right-0 bottom-full mb-1 bg-popover text-popover-foreground rounded-xl border border-border shadow-lg py-1 z-50 max-h-56 overflow-y-auto">
          {options.map((opt) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2 text-left text-sm transition-colors ${isSelected ? "bg-brand-50 text-brand-800" : "hover:bg-muted text-foreground"}`}
              >
                {opt.icon && <opt.icon className={`h-4 w-4 shrink-0 ${isSelected ? "text-brand-600" : "text-muted-foreground"}`} />}
                <div className="flex-1 min-w-0">
                  <span className={`block truncate ${isSelected ? "font-medium" : ""}`}>{opt.label}</span>
                  {opt.desc && <span className="block text-[10px] text-muted-foreground truncate">{opt.desc}</span>}
                </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {hooks.length === 0 && !showForm ? (
            <div className="text-center py-16">
              ...
              <h3 className="text-sm font-semibold text-foreground">Sin hooks configurados</h3>
            ...
            <div className="space-y-3">
              {hooks.map((hook) => (
                <div key={hook.id} className={`bg-card rounded-xl border p-4 flex items-center gap-4 transition-colors ${hook.enabled ? "border-border" : "border-border opacity-60"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {hook.config.conversionName && (
                      <p className="text-xs text-muted-foreground mt-1">Conversión: {hook.config.conversionName}</p>
                    )}
                    ...
                    {hook.config.tagName && (
                      <p className="text-xs text-muted-foreground mt-1">Etiqueta: {hook.config.tagName}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEditForm(hook)} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-muted-foreground transition-colors">
                      <Edit3 className="h-4 w-4" />
                    </button>
                    <button onClick={() => handleDelete(hook.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-muted-foreground hover:text-red-500 transition-colors">
              ...
              <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-muted/50">
                <div>
                  <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
                    {editingId ? <Edit3 className="h-4 w-4 text-muted-foreground" /> : <Plus className="h-4 w-4 text-brand-600" />}
                    {editingId ? "Editar hook" : "Nuevo hook"}
                  </h3>
                  <p className="text-[11px] text-muted-foreground mt-0.5">Define qué pasa cuando ocurre un evento en tu tienda</p>
                </div>
                <button onClick={() => setShowForm(false)} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground transition-colors">
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <label className="text-xs font-bold text-foreground uppercase tracking-wide">¿Qué evento quieres capturar?</label>
                  </div>
                        ...
                        <button
                          ...
                          className={`group text-left p-3.5 rounded-xl border-2 transition-all duration-200 relative ${
                            !evt.active
                              ...
                              : "border-border hover:border-purple-200 hover:bg-purple-50/30 hover:shadow-sm"
                          }`}
                        >
                          <div className="flex items-start gap-3">
                            <div className={`h-8 w-8 rounded-lg ${evt.bg} flex items-center justify-center shrink-0 transition-transform ${evt.active ? "group-hover:scale-110" : ""}`}>
                              <EvtIcon className={`h-4 w-4 ${evt.active ? evt.color : "text-muted-foreground"}`} />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <span className="text-xs font-semibold text-foreground block">{evt.label}</span>
                                {!evt.active && (
                                  <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground font-medium">Próximamente</span>
                                )}
                              </div>
                              <span className="block text-[11px] text-muted-foreground mt-0.5 leading-relaxed">{evt.description}</span>
                              {evt.active && (
                                <span className={`block text-[10px] mt-1.5 italic transition-opacity ${formEvent === evt.key ? "text-purple-600 opacity-100" : "text-muted-foreground opacity-0 group-hover:opacity-100"}`}>
                                  Ej: {evt.example}
                                </span>
                              )}
                            </div>
                ...
                <div className={`transition-opacity duration-300 ${formEvent ? "opacity-100" : "opacity-40 pointer-events-none"}`}>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <label className="text-xs font-bold text-foreground uppercase tracking-wide">¿Qué acción ejecutar?</label>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className={`h-9 w-9 rounded-lg ${act.bg} flex items-center justify-center mb-2.5 transition-transform ${act.active ? "group-hover:scale-110" : ""}`}>
                            <Icon className={`h-4.5 w-4.5 ${act.active ? act.color : "text-muted-foreground"}`} />
                          </div>
                          <div className="flex items-center gap-2">
                          ...
                          <span className="block text-[11px] text-muted-foreground mt-1 leading-relaxed">{act.description}</span>
                          {act.active && (
                            <span className={`block text-[10px] mt-2 font-medium transition-opacity ${isSelected ? `${act.color} opacity-100` : "text-muted-foreground opacity-0 group-hover:opacity-100"}`}>
                              {act.tip}
                            </span>
                          )}
                ...
                {formAction && (
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <label className="text-xs font-bold text-foreground uppercase tracking-wide">Configuración</label>
                    </div>
                    ...
                    <div className="bg-muted rounded-xl p-4 border border-border">
                      {formAction === "conversion" && (
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {([
                                ...
                                { value: "custom", label: "Otro", desc: "Evento personalizado", icon: Tag, color: "text-muted-foreground", bg: "bg-muted" },
                              ]).map((opt) => {
                                const Icon = opt.icon;
                                ...
                                return (
                                  ...
                                  >
                                    <div className={`h-7 w-7 rounded-lg ${opt.bg} flex items-center justify-center shrink-0 transition-transform group-hover:scale-110`}>
                                      <Icon className={`h-3.5 w-3.5 ${isSelected ? opt.color : "text-muted-foreground"}`} />
                                    </div>
                                    <div>
                                      <p className={`text-xs font-semibold ${isSelected ? "text-foreground" : "text-foreground"}`}>{opt.label}</p>
                          ...
                          {/* Event name */}
                          <div>
                            <label className="text-[11px] font-medium text-foreground">Nombre del evento</label>
                            <input
                              type="text"
                              ...
                              className="w-full mt-1 border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-200 focus:border-amber-400 bg-card"
                            />
                            <p className="text-[10px] text-muted-foreground mt-1">Puedes usar variables de WooCommerce arrastrándolas al campo</p>
                          </div>

                          {/* Value */}
                          <div>
                            <label className="text-[11px] font-medium text-foreground">Valor monetario <span className="text-muted-foreground font-normal">(opcional)</span></label>
                            <div className="flex gap-2 mt-1">
                              <input
                                ...
                                className="flex-1 border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-200 focus:border-amber-400 bg-card"
                              />
                              <Dropdown
                            ...
                            <p className="text-[10px] text-muted-foreground mt-1">Usa {"{{total}}"} para tomar el valor automáticamente del pedido</p>
                          </div>

                          {/* Available variables */}
                          <div>
                            <label className="text-[11px] font-medium text-foreground mb-2 block">Variables disponibles de WooCommerce</label>
                            <p className="text-[10px] text-muted-foreground mb-2">Arrastra o haz clic para copiar e insertar en los campos</p>
                      ...
                      {formAction === "notification" && (
                        <div className="space-y-4">
                          ...
                          {formConfig.channel === "whatsapp" && formConfig.inboxId && (
                            <div className="space-y-3">
                              <div>
                                <label className="text-xs font-medium text-foreground mb-2 block">Plantilla aprobada</label>
                                {loadingTpl ? (
                                  <div className="flex items-center gap-2 text-xs text-muted-foreground py-3">
                                    <Loader2 className="h-3.5 w-3.5 animate-spin" /> Cargando plantillas...
                                  </div>
                                ) : whatsappTemplates.length === 0 ? (
                                  <p className="text-xs text-muted-foreground py-2">No hay plantillas aprobadas en esta bandeja.</p>
                                ) : (
                                  <Dropdown
                              ...
                              {formConfig.templateName && formConfig.variableMapping && Object.keys(formConfig.variableMapping).length > 0 && (
                                <div>
                                  <label className="text-xs font-medium text-foreground mb-1 block">Mapeo de variables</label>
                                  <p className="text-[10px] text-muted-foreground mb-2">Asigna datos de WooCommerce a cada variable de la plantilla</p>
                                  <div className="space-y-2">
                                    {Object.keys(formConfig.variableMapping).sort((a, b) => Number(a) - Number(b)).map((varNum) => (
                                      <div key={varNum} className="flex items-center gap-2 p-2.5 rounded-lg bg-muted border border-border">
                                        <span className="text-xs font-mono text-muted-foreground w-12 shrink-0">{`{{${varNum}}}`}</span>
                          ...
                          {formConfig.channel && formConfig.channel !== "whatsapp" && (
                            <div>
                              ...
                              <textarea
                                ...
                                className="w-full border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400 resize-none bg-card"
                              />
                      ...
                      {formAction === "tag" && (
                        <div>
                          ...
                          <input
                            ...
                            className="w-full border border-border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-200 focus:border-green-400 bg-card"
                          />
                          <div className="mt-3 flex items-start gap-2 text-[11px] text-muted-foreground bg-green-50 border border-green-100 rounded-lg p-2.5">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-border">
                  <button onClick={() => setShowForm(false)} className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-colors">
                    Cancelar
                  </button>
                  <button
                    onClick={handleSave}
