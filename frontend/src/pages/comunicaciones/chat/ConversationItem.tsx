...
export const ConversationItem = memo(function ConversationItem({
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 text-left border-b border-border transition-colors ${isActive ? "bg-brand-50 dark:bg-brand-700/40" : "hover:bg-muted"}`}
    >
      <div className={`relative h-10 w-10 rounded-full flex items-center justify-center text-sm font-bold text-muted-foreground shrink-0 ${conv.hasAdTracking ? "ring-2 ring-blue-500 ring-offset-1 bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-blue-500/20 dark:to-indigo-500/20" : "bg-muted"}`}>
        {displayName.charAt(0).toUpperCase()}
        {conv.hasAdTracking && conv.adPlatform && (
          <AdPlatformBadge platform={conv.adPlatform} />
        )}
        {assignedMember && (
          <span className="absolute -top-1 -left-1 h-4.5 w-4.5 rounded-full bg-brand-600 text-white text-[8px] font-bold flex items-center justify-center border-2 border-card shadow-sm" title={assignedMember.user.name}>
        ...
        {conv.inbox && (
          <p className="text-[10px] text-muted-foreground flex items-center gap-1 mb-0.5">
            ...
            {conv.lastMessageSource === "campaign" && (
              <span className="inline-flex items-center gap-0.5 px-1.5 py-0 rounded-full bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-300 text-[9px] font-medium ml-1">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 shrink-0 ml-2">
            {conv.botStatus === "handed_off" && (
              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-300 font-medium">Agente</span>
            )}
            {normalizeConvStatus(conv.status) === "resolved" && (
              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 dark:bg-green-500/15 dark:text-green-300 font-medium">Resuelta</span>
            )}
            {normalizeConvStatus(conv.status) === "archived" && (
              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300 font-medium">Archivada</span>
            )}
...
function AdPlatformBadge({ platform }: { platform: string }) {
  return (
    <span className="absolute -bottom-0.5 -right-0.5 h-4 w-4 rounded-full bg-card border border-border flex items-center justify-center shadow-sm">
