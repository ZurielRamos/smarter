      ...
      <div className="max-w-2xl">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">Define cuándo el canal está disponible para recibir y responder mensajes</p>
          </div>
          <button
            onClick={handleSave}
            disabled={saving}
              ...
              >
                ...
                <button
                  ...
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors shrink-0 ${
                    day.enabled ? "bg-brand-600" : "bg-gray-300 dark:bg-gray-600"
                  }`}
                >
                ...
                {day.enabled ? (
                  <div className="flex items-center gap-2 flex-1">
                    <input
                      ...
                      className="px-2 py-1 rounded-md border border-border text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-brand-500"
                    />
        ...
        <div className="mt-4 p-3 rounded-lg bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/25">
  ...
  return (
    <div className="flex-1 overflow-y-auto p-5">
      <div className="max-w-2xl space-y-6">
        {/* Assigned agents */}
        <div>
          <h3 className="text-xs font-semibold text-muted-foreground uppercase mb-2">Agentes asignados</h3>
          {assignedUserIds.length === 0 ? (
            <p className="text-xs text-muted-foreground py-2">Sin agentes asignados</p>
          ) : (
                ...
                return (
                  <div key={c.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-muted border border-border">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">{agent?.user.name || "Usuario"}</p>
                      <p className="text-[10px] text-muted-foreground truncate">{agent?.user.email}</p>
                    </div>
                    <button onClick={() => handleRemove(c.id)} className="p-1 rounded hover:bg-red-50 dark:hover:bg-red-500/10 text-muted-foreground hover:text-red-500">
        ...
        {/* Assigned teams */}
        <div>
          <h3 className="text-xs font-semibold text-muted-foreground uppercase mb-2">Equipos asignados</h3>
          {assignedTeamIds.length === 0 ? (
            <p className="text-xs text-muted-foreground py-2">Sin equipos asignados</p>
          ) : (
            <div className="space-y-1">
...
function WhatsAppStatusTab({ inboxId }: { inboxId: string }) {
  ...
  if (loading) return <div className="flex-1 flex items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></div>;
  if (!data) return <div className="flex-1 flex items-center justify-center text-sm text-muted-foreground">No se pudo obtener el estado</div>;
      ...
      <div className="max-w-2xl space-y-5">
        ...
        <div className="bg-card rounded-xl border border-border p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            {!editingProfile ? (
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setEditingProfile(false)} className="text-xs text-muted-foreground hover:text-foreground">Cancelar</button>
          ...
          <div className="flex items-start gap-4 mb-4 pb-4 border-b border-border">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-base font-semibold text-foreground">{phone.verified_name || "Sin nombre verificado"}</h3>
                ...
                <div className="mt-2">
                  {!showRegister ? (
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <input type="text" value={registerPin} onChange={(e) => setRegisterPin(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="PIN de 6 dígitos" maxLength={6} className="w-32 px-2.5 py-1.5 rounded-lg border border-border bg-background text-foreground placeholder:text-muted-foreground text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
                      ...
                      <button onClick={() => setShowRegister(false)} className="text-xs text-muted-foreground hover:text-foreground">Cancelar</button>
                    </div>
                  )}
                </div>
              )}
          ...
          {editingProfile ? (
            <div className="space-y-3">
              <div>
                <label className="text-[10px] text-muted-foreground uppercase font-medium mb-0.5 block">Acerca de</label>
                <input type="text" value={profileForm.about} onChange={(e) => setProfileForm({ ...profileForm, about: e.target.value })} placeholder="Breve descripción de tu negocio" maxLength={139} className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground placeholder:text-muted-foreground text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
              </div>
              <div>
                <label className="text-[10px] text-muted-foreground uppercase font-medium mb-0.5 block">Descripción</label>
                <textarea value={profileForm.description} onChange={(e) => setProfileForm({ ...profileForm, description: e.target.value })} placeholder="Descripción detallada del negocio" maxLength={512} rows={3} className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground placeholder:text-muted-foreground text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-muted-foreground uppercase font-medium mb-0.5 block">Dirección</label>
                  <input type="text" value={profileForm.address} onChange={(e) => setProfileForm({ ...profileForm, address: e.target.value })} placeholder="Dirección del negocio" className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground placeholder:text-muted-foreground text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
                </div>
                <div>
                  <label className="text-[10px] text-muted-foreground uppercase font-medium mb-0.5 block">Email</label>
                  <input type="email" value={profileForm.email} onChange={(e) => setProfileForm({ ...profileForm, email: e.target.value })} placeholder="contacto@empresa.com" className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground placeholder:text-muted-foreground text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
                </div>
              </div>
              <div>
            ...
            <div className="space-y-2.5">
              {profile?.about && <div><p className="text-[10px] text-muted-foreground uppercase font-medium mb-0.5">Acerca de</p><p className="text-sm text-foreground">{profile.about}</p></div>}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-semibold text-muted-foreground uppercase">Tier de mensajes</span>
            </div>
            <p className="text-lg font-bold text-foreground">{tierLabels[phone.messaging_limit_tier] || phone.messaging_limit_tier || "No disponible"}</p>
            <p className="text-[10px] text-muted-foreground mt-0.5">Límite de conversaciones iniciadas por negocio</p>
          </div>
        </div>
        ...
        <div className="bg-card rounded-xl border border-border p-5">
          <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
            <Shield className="h-4 w-4 text-muted-foreground" /> Verificación
          </h4>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] text-muted-foreground uppercase font-medium mb-1">Estado del nombre</p>
              {phone.name_status ? (
                <span className={`text-xs px-2 py-1 rounded font-medium ${nameStatusLabels[phone.name_status]?.color || "bg-muted text-foreground"}`}>
            ...
            <div>
              <p className="text-[10px] text-muted-foreground uppercase font-medium mb-1">Verificación de negocio</p>
              {waba.business_verification_status ? (
                <span className={`text-xs px-2 py-1 rounded font-medium ${verificationLabels[waba.business_verification_status]?.color || "bg-muted text-foreground"}`}>
        ...
        {waba.name && (
          <div className="bg-card rounded-xl border border-border p-5">
            <h4 className="text-sm font-semibold text-foreground mb-3">Cuenta de WhatsApp Business (WABA)</h4>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Nombre</span>
                <span className="text-foreground font-medium">{waba.name}</span>
              </div>
              {waba.currency && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Moneda</span>
                  <span className="text-foreground">{waba.currency}</span>
                </div>
              )}
              {waba.ownership_type && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Tipo de propiedad</span>
                  <span className="text-foreground">{waba.ownership_type}</span>
                </div>
              )}
              {waba.message_template_namespace && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Namespace de plantillas</span>
                  <span className="text-foreground font-mono text-xs">{waba.message_template_namespace}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="bg-muted rounded-xl border border-border p-4">
          ...
          <div className="space-y-1 text-xs font-mono">
            ...
            {phone.platform_type && <div className="flex justify-between"><span className="text-muted-foreground">Plataforma</span><span className="text-foreground">{phone.platform_type}</span></div>}
            {phone.account_mode && <div className="flex justify-between"><span className="text-muted-foreground">Modo de cuenta</span><span className="text-foreground">{phone.account_mode}</span></div>}
          </div>
        </div>
      </div>
    </div>
...
function BotsTab({ inboxId, tenantId }: { inboxId: string; tenantId: string }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-5">
      <div className="max-w-2xl">
        <div className="mb-4">
          <h3 className="text-sm font-semibold text-foreground">Bot de respuesta automática</h3>
          <p className="text-[11px] text-muted-foreground mt-0.5">Asigna un bot para que responda automáticamente los chats entrantes de este canal</p>
        </div>
        ...
        {assignedBot ? (
          <div className="bg-card rounded-xl border border-green-200 p-4 mb-4">
            ...
            {assignedBot.status !== "active" && (
              <div className="mt-3 p-2.5 rounded-lg bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/25">
          ...
          <div className="bg-muted rounded-xl border border-dashed border-border p-6 mb-4 text-center">
            <Bot className="h-8 w-8 text-muted-foreground mx-auto mb-2" />
            <p className="text-sm text-muted-foreground">Sin bot asignado</p>
            <p className="text-[11px] text-muted-foreground mt-0.5">Selecciona un bot de la lista para activar respuestas automáticas</p>
          </div>
        )}
        ...
        {/* Available bots list */}
        <div>
          <h4 className="text-xs font-semibold text-muted-foreground uppercase mb-2">Bots disponibles</h4>
          {bots.length === 0 ? (
            <div className="text-center py-6">
              <p className="text-xs text-muted-foreground">No hay bots creados</p>
              <p className="text-[11px] text-muted-foreground mt-1">Crea un bot en el módulo de Bots para poder asignarlo aquí</p>
            </div>
          ) : (
            <div className="space-y-2">
              {bots.map((bot) => {
