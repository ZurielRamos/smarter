...
import { Users, Database, Upload, Settings2, Columns, Eye, EyeOff, ListOrdered, RotateCcw, ArrowUpNarrowWide, ArrowDownNarrowWide, Filter, Plus, Loader2, ChevronDown, List, Edit3, Copy, UserX, Trash2, X, Save, LayoutGrid, TableProperties, StickyNote, UserPlus, Download, Phone } from "lucide-react";
...
const NormalizePhonesModal = lazy(() => import("@/components/NormalizePhonesModal").then((m) => ({ default: m.NormalizePhonesModal })));
const DetectDuplicatesModal = lazy(() => import("@/components/DetectDuplicatesModal").then((m) => ({ default: m.DetectDuplicatesModal })));
...
const SYSTEM_COLUMNS: ColumnDef[] = [
  ...
  { key: "jobTitle", label: "Cargo", visible: false },
  ...
  { key: "language", label: "Idioma", visible: false },
  ...
  { key: "notes", label: "Notas rápidas", visible: false },
...
export function Clients() {
  ...
  const [normalizePhonesOpen, setNormalizePhonesOpen] = useState(false);
  const [detectDuplicatesOpen, setDetectDuplicatesOpen] = useState(false);
  ...
  const getCellValue = (client: ClientRecord, key: string) => {
    ...
    switch (key) {
      case "id":
        return <span className="font-mono text-xs text-muted-foreground">{client.id?.slice(0, 8)}...</span>;
      case "name":
      ...
      case "tags":
        return client.tags && client.tags.length > 0
          ? client.tags.map((tag) => (
              <span key={tag} className="inline-block mr-1 px-2 py-0.5 rounded-full text-xs bg-brand-100 text-brand-700 dark:bg-brand-700 dark:text-brand-100">{tag}</span>
            ))
          : "—";
      default: {
    ...
    <div className="h-full flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={listsDropdownRef}>
              ...
              {listsDropdownOpen && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-popover text-popover-foreground rounded-lg shadow-lg border border-border py-1 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                  {activeList && (
                    <>
                      <button
                        onClick={() => { setActiveList(null); setListsDropdownOpen(false); }}
                        className="flex items-center gap-2 w-full px-3 py-2 text-sm text-brand-600 hover:bg-muted transition-colors font-medium"
                      >
                  ...
                  {recordLists.length > 0 ? (
                    <>
                      <p className="px-3 py-1 text-[10px] font-semibold text-muted-foreground uppercase">Mis listas</p>
                      {recordLists.map((list) => (
                        <div
                          key={list.id}
                          className={`flex items-center justify-between w-full px-3 py-2 transition-colors group ${activeList?.id === list.id ? "bg-brand-50 dark:bg-brand-700" : "hover:bg-muted"}`}
                        >
                          <button
                            onClick={() => { setActiveList(list); setListsDropdownOpen(false); }}
                            className={`flex-1 text-left text-sm ${activeList?.id === list.id ? "text-brand-700 dark:text-brand-100" : "text-foreground"}`}
                          >
                            {list.name}
                          </button>
                          <div className="flex items-center gap-1.5">
                            <span className="text-[10px] text-muted-foreground">{list.type === "dynamic" ? "Dinámica" : "Estática"}</span>
                            <button
                    ...
                    <p className="px-3 py-2 text-xs text-muted-foreground">Sin listas creadas</p>
                  )}
                  <button
            ...
            <div className="relative flex" ref={dropdownRef}>
              ...
              {dropdownOpen && (
                <div
                  className="absolute right-0 top-full mt-2 w-52 bg-popover text-popover-foreground rounded-lg shadow-lg border border-border py-1 z-50 animate-in fade-in slide-in-from-top-1 duration-150"
                >
                  <button
                    onClick={() => {
                      setDropdownOpen(false);
                  ...
                  <div className="border-t border-border my-1" />
                  <p className="px-3 py-1 text-[10px] font-semibold text-muted-foreground uppercase">Sincronizar</p>
                  <button
      ...
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden pt-4 px-0">
        ...
        {(!mounted || (loading && viewMode === "list")) ? (
          ...
          <div className="flex-1 min-h-0 flex flex-col items-center justify-center bg-card rounded-xl border border-border overflow-hidden">
                ...
                <thead className="sticky top-0 z-10">
                  <tr className="bg-muted text-muted-foreground border-b border-border">
                    ...
                    {visibleColumns.map((col, colIndex) => col ? (
                      <th
                        key={col.key}
                        className="relative px-4 py-3 text-left font-semibold text-foreground whitespace-nowrap"
                      ...
                      >
                        <div className="flex items-center justify-between gap-1">
                          ...
                          <div className="relative">
                            <button
                              ...
                              className={`p-0.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-all ${hoveredCol === col.key || colMenuOpen === col.key ? 'opacity-100' : 'opacity-0'}`}
                            >
                            ...
                            {colMenuOpen === col.key && (
                              <div data-col-menu className={`absolute top-full mt-1 w-44 bg-popover text-popover-foreground rounded-lg shadow-lg border border-border py-1.5 z-50 ${colIndex === 0 ? 'left-0' : 'right-0'}`}>
                            <button
                              onClick={() => { setColMenuOpen(null); setSearchParams({ page: "1", limit: String(limit), sortBy: col.key, sortOrder: "ASC" }); }}
                              className={`flex items-center gap-2.5 w-full px-3 py-2 text-sm transition-colors ${sortBy === col.key && sortOrder === "ASC" ? 'bg-brand-50 text-brand-700 dark:bg-brand-700 dark:text-brand-100' : 'text-foreground hover:bg-muted'}`}
                            >
                            ...
                            <button
                              ...
                              className={`flex items-center gap-2.5 w-full px-3 py-2 text-sm transition-colors ${sortBy === col.key && sortOrder === "DESC" ? 'bg-brand-50 text-brand-700 dark:bg-brand-700 dark:text-brand-100' : 'text-foreground hover:bg-muted'}`}
                            >
                            ...
                            <button
                              ...
                              className="flex items-center gap-2.5 w-full px-3 py-2 text-sm text-muted-foreground hover:bg-muted transition-colors"
                            >
                        ...
                        <div
                          ...
                          className={`absolute right-0 top-0 bottom-0 w-1 cursor-col-resize transition-colors ${hoveredCol === col.key ? 'bg-border' : 'bg-transparent'}`}
                        />
                      </th>
                    ) : null)}
                    <th className="px-2 py-3 text-right w-10 sticky right-0 bg-muted">
                      <div className="relative" ref={tableMenuRef}>
                        <button onClick={() => setTableMenuOpen((v) => !v)} className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors">
                          <Settings2 className="h-4 w-4" />
                        </button>
                        {tableMenuOpen && (
                            <div
                              className="absolute right-0 top-full mt-1 w-60 bg-popover text-popover-foreground rounded-lg shadow-lg border border-border py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-150"
                            >
                              <button
                              ...
                              <div className="relative group/limit">
                                ...
                                <div className="hidden group-hover/limit:block absolute right-full top-0 w-36 bg-popover text-popover-foreground rounded-lg shadow-lg border border-border py-2 z-50">
                                  {[10, 25, 50, 100, 200].map((n) => (
                                    <button
                                      key={n}
                                      onClick={() => { setTableMenuOpen(false); setSearchParams({ page: "1", limit: String(n), ...(sortBy && { sortBy }), ...(sortOrder && { sortOrder }) }); }}
                                      className={`w-full px-3 py-1.5 text-sm text-left transition-colors ${limit === n ? 'bg-brand-50 text-brand-700 dark:bg-brand-700 dark:text-brand-100 font-medium' : 'text-foreground hover:bg-muted'}`}
                                    >
                                      {n} contactos
                                    </button>
                                  ))}
                              ...
                              <div className="flex items-center justify-between px-4 py-2.5">
                                <div className="flex items-center gap-2.5">
                                  ...
                                  <span className="text-sm text-foreground">Modo de vista</span>
                                </div>
                                <span className="text-xs font-medium text-muted-foreground">Ajustar texto</span>
                              </div>
                            </div>
                          )}
                      </div>
        ...
        {viewMode === "list" && totalPages > 0 && (
          <div className="shrink-0 flex items-center justify-between px-4 py-3 border-t border-border bg-card rounded-b-xl">
      ...
      {contextMenu && (
        <div
          className="fixed z-50 bg-popover text-popover-foreground border border-border rounded-lg shadow-lg py-1 min-w-[180px]"
          style={{ top: contextMenu.y, left: contextMenu.x }}
        >
          <button onClick={() => { navigate(`/${slug}/clients/${contextMenu.client.id}`); setContextMenu(null); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-foreground hover:bg-muted">
            <Eye className="h-3.5 w-3.5 text-muted-foreground" /> Ver detalles
          </button>
          <button onClick={() => { setEditingClient(contextMenu.client); setContextMenu(null); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-foreground hover:bg-muted">
            <Edit3 className="h-3.5 w-3.5 text-muted-foreground" /> Editar contacto
          </button>
          <button onClick={() => { navigator.clipboard.writeText(contextMenu.client.phone || ""); setContextMenu(null); toast.success("Teléfono copiado"); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-foreground hover:bg-muted">
            <Copy className="h-3.5 w-3.5 text-muted-foreground" /> Copiar teléfono
          </button>
          <button onClick={() => { navigator.clipboard.writeText(contextMenu.client.email || ""); setContextMenu(null); toast.success("Email copiado"); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-foreground hover:bg-muted">
            <Copy className="h-3.5 w-3.5 text-muted-foreground" /> Copiar email
          </button>
          <button onClick={() => { setNoteClient(contextMenu.client); setContextMenu(null); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-foreground hover:bg-muted">
            <StickyNote className="h-3.5 w-3.5 text-muted-foreground" /> Agregar nota
          </button>
          <div
            className="relative"
          ...
          >
            <button className="w-full flex items-center justify-between px-3 py-2 text-sm text-foreground hover:bg-muted">
              <span className="flex items-center gap-2"><UserPlus className="h-3.5 w-3.5 text-muted-foreground" /> Asignar agente</span>
          ...
          <div className="border-t border-border my-1" />
          <button onClick={() => { setSingleDeleteId(contextMenu.client.id); setContextMenu(null); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10">
      ...
      {normalizePhonesOpen && (
        <Suspense fallback={null}>
          <NormalizePhonesModal
            tenantId={tenantId}
            onClose={() => setNormalizePhonesOpen(false)}
            onApplied={() => loadClients()}
          />
        </Suspense>
      )}

      {detectDuplicatesOpen && (
        <Suspense fallback={null}>
          <DetectDuplicatesModal
            tenantId={tenantId}
            onClose={() => setDetectDuplicatesOpen(false)}
            onMerged={() => loadClients()}
          />
        </Suspense>
      )}
    </div>
...
function AssignAgentSubmenu({ agents, teams, currentAssignee, currentTeam, onAssign, onAssignTeam }: {
  ...
  return (
    <div className="absolute left-full top-0 ml-1 w-60 bg-popover text-popover-foreground rounded-lg shadow-lg border border-border py-1 z-50">
      <div className="px-2.5 pb-1.5 pt-1">
        <input
          ...
          className="w-full px-2.5 py-1.5 rounded-md border border-border bg-background text-foreground placeholder:text-muted-foreground text-xs focus:outline-none focus:ring-1 focus:ring-brand-500"
          autoFocus
      ...
      <div className="max-h-64 overflow-auto">
        ...
        {(currentAssignee || currentTeam) && (
          <>
            <button
              ...
              className="w-full px-3 py-2 text-xs text-left text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors"
            >
              Quitar asignación
            </button>
            <div className="border-t border-border my-1" />
        ...
        {filteredTeams.length > 0 && (
          <>
            <p className="px-3 py-1 text-[10px] font-semibold text-muted-foreground uppercase">Equipos</p>
            {filteredTeams.map((team) => (
              <button
                key={team.id}
                onClick={(e) => { e.stopPropagation(); onAssignTeam(team.id); }}
                className={`w-full flex items-center gap-2.5 px-3 py-2 text-left transition-colors ${currentTeam === team.id ? "bg-brand-50 dark:bg-brand-700" : "hover:bg-muted"}`}
              >
                <div className="h-6 w-6 rounded-md bg-indigo-100 dark:bg-indigo-500/10 flex items-center justify-center text-[10px] font-semibold text-indigo-600 dark:text-indigo-400 shrink-0">
                  {team.name[0]?.toUpperCase() || "E"}
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-xs truncate ${currentTeam === team.id ? "font-medium text-brand-700 dark:text-brand-100" : "text-foreground"}`}>{team.name}</p>
                  {team.description && <p className="text-[10px] text-muted-foreground truncate">{team.description}</p>}
                </div>
                {currentTeam === team.id && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-brand-100 text-brand-700 dark:bg-brand-700 dark:text-brand-100 font-medium shrink-0">Actual</span>
                )}
              </button>
            ))}
            <div className="border-t border-border my-1" />
        ...
        {/* Agents */}
        <p className="px-3 py-1 text-[10px] font-semibold text-muted-foreground uppercase">Agentes</p>
        {filteredAgents.length === 0 ? (
          ...
          filteredAgents.map((agent) => (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 text-left transition-colors ${currentAssignee === agent.userId ? "bg-brand-50 dark:bg-brand-700" : "hover:bg-muted"}`}
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-xs truncate ${currentAssignee === agent.userId ? "font-medium text-brand-700 dark:text-brand-100" : "text-foreground"}`}>{agent.user.name}</p>
...
function KanbanFieldMenu({
  ...
  return (
    <div ref={menuRef} className="absolute right-0 top-full mt-2 w-52 bg-popover text-popover-foreground rounded-lg shadow-lg border border-border py-1 z-50">
      {fields.map((f) => (
        <div
          key={f.key}
          className="relative"
        ...
        >
          <button
            ...
            className={`w-full px-3 py-2 text-sm text-left transition-colors flex items-center justify-between ${
              activeField === f.key ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 font-medium" : "text-foreground hover:bg-muted"
            }`}
          >
            <span>{f.label}</span>
          ...
          {hoveredField === f.key && activeField === f.key && (
            <div className="absolute left-full top-0 ml-1 w-52 bg-popover text-popover-foreground rounded-lg shadow-lg border border-border py-1 max-h-64 overflow-auto">
              <p className="px-3 py-1.5 text-[10px] font-semibold text-muted-foreground uppercase">Columnas</p>
              {allOptions().map((option) => {
                const currentVisible = visibleColumns[activeField];
                const isVisible = !currentVisible || currentVisible.includes(option);
                return (
                  <label key={option} className="flex items-center justify-between px-3 py-1.5 hover:bg-muted cursor-pointer">
                    ...
                    <button
                      ...
                      className={`relative w-8 h-[18px] rounded-full transition-colors ${isVisible ? "bg-emerald-500" : "bg-gray-300 dark:bg-gray-600"}`}
                    >
