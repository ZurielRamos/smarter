...
export function KanbanView({ tenantId, groupByField, fieldOptions, fieldLabel, assignedTo, assignedTeamId, onMoveClient, onClientClick, onContextMenu }: KanbanViewProps) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="shrink-0 px-4 py-2 text-xs text-muted-foreground border-t border-border bg-card">
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-border shrink-0">
        ...
        <div className="ml-auto relative" ref={menuRef}>
          <button
            ...
            className={`h-6 w-6 rounded flex items-center justify-center transition-colors ${hasActiveFilter ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
          >
            <SlidersHorizontal className="h-3.5 w-3.5" />
          </button>
          {menuOpen && (
            <div className="absolute right-0 top-full mt-1 w-56 bg-popover text-popover-foreground rounded-lg shadow-lg border border-border py-2 z-50">
              <div className="px-2.5 pb-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-7 pr-7 py-1.5 rounded-md border border-border bg-background text-foreground placeholder:text-muted-foreground text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    autoFocus
                  />
                  {localSearch && (
              ...
              <p className="px-3 py-1 text-[10px] font-semibold text-muted-foreground uppercase">Ordenar por</p>
              {SORT_OPTIONS.map((opt) => (
                <button key={opt.value} onClick={() => handleSortChange(opt.value)}
                  className={`w-full px-3 py-1.5 text-xs text-left transition-colors ${localSort === opt.value ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 font-medium" : "text-muted-foreground hover:bg-muted"}`}
                >{opt.label}</button>
              ))}
              {hasActiveFilter && (
                <>
                  <div className="border-t border-border my-1" />
                  <button onClick={handleClear} className="w-full px-3 py-1.5 text-xs text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 text-left">Limpiar filtros</button>
                </>
              )}
            </div>
          )}
        ...
        {state.loading ? (
          ...
          <>
            ...
            {state.clients.length === 0 && (
              <div className="flex items-center justify-center py-8 text-xs text-muted-foreground">{localSearch ? "Sin resultados" : "Sin contactos"}</div>
            )}
            {hasMore && (
              <button onClick={onLoadMore} disabled={state.loadingMore} className="w-full py-2 text-xs text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 font-medium hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-lg transition-colors disabled:opacity-50">
...
function KanbanCard({ client, isDragging, onDragStart, onDragEnd, onClick, onContextMenu }: {
  ...
  return (
    <div draggable onDragStart={(e) => onDragStart(e, client.id)} onDragEnd={onDragEnd} onClick={onClick}
      ...
      className={`group bg-card rounded-lg border border-border p-3 cursor-pointer hover:border-gray-300 hover:shadow-sm transition-all ${isDragging ? "opacity-50 scale-95" : ""}`}
    >
      <div className="flex items-start gap-2.5">
        <div className={`relative h-8 w-8 rounded-full flex items-center justify-center text-xs font-semibold text-muted-foreground shrink-0 ${client.hasAdTracking ? "ring-2 ring-blue-500 ring-offset-1 bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-blue-500/10 dark:to-indigo-500/10" : "bg-muted"}`}>
          {(client.firstName?.[0] || client.email?.[0] || "?").toUpperCase()}
          {client.hasAdTracking && client.adLastPlatform && (
            <span className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full bg-card border border-border flex items-center justify-center shadow-sm">
        ...
        <div className="flex-1 min-w-0">
          ...
          {client.email && <p className="text-xs text-muted-foreground truncate mt-0.5 flex items-center gap-1"><Mail className="h-3 w-3 shrink-0" /><span className="truncate">{client.email}</span></p>}
          {client.phone && <p className="text-xs text-muted-foreground truncate mt-0.5 flex items-center gap-1"><Phone className="h-3 w-3 shrink-0" />{client.phone}</p>}
        </div>
      ...
      {client.tags && client.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-2">
          {client.tags.slice(0, 3).map((tag) => (<span key={tag} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] bg-muted text-muted-foreground"><Tag className="h-2.5 w-2.5" />{tag}</span>))}
          {client.tags.length > 3 && <span className="text-[10px] text-muted-foreground">+{client.tags.length - 3}</span>}
        </div>
      )}
      {client.score > 0 && (
        <div className="mt-2 flex items-center gap-1.5">
          <div className="flex-1 h-1 rounded-full bg-muted overflow-hidden"><div className="h-full rounded-full bg-emerald-500" style={{ width: `${Math.min(client.score, 100)}%` }} /></div>
          <span className="text-[10px] text-muted-foreground font-medium">{client.score}</span>
        </div>
      )}
    </div>
  );
