...
const statusColors: Record<string, { bg: string; text: string; label: string }> = {
  lead: { bg: "bg-blue-100 dark:bg-blue-500/15", text: "text-blue-700 dark:text-blue-300", label: "Lead" },
  contactado: { bg: "bg-sky-100 dark:bg-sky-500/15", text: "text-sky-700 dark:text-sky-300", label: "Contactado" },
  interesado: { bg: "bg-indigo-100 dark:bg-indigo-500/15", text: "text-indigo-700 dark:text-indigo-300", label: "Interesado" },
  oportunidad: { bg: "bg-amber-100 dark:bg-amber-500/15", text: "text-amber-700 dark:text-amber-300", label: "Oportunidad" },
  cliente: { bg: "bg-green-100 dark:bg-green-500/15", text: "text-green-700 dark:text-green-300", label: "Cliente" },
  premium: { bg: "bg-purple-100 dark:bg-purple-500/15", text: "text-purple-700 dark:text-purple-300", label: "Premium" },
  fidelizado: { bg: "bg-emerald-100 dark:bg-emerald-500/15", text: "text-emerald-700 dark:text-emerald-300", label: "Fidelizado" },
  inactivo: { bg: "bg-muted", text: "text-muted-foreground", label: "Inactivo" },
  perdido: { bg: "bg-red-100 dark:bg-red-500/15", text: "text-red-700 dark:text-red-300", label: "Perdido" },
  // Legacy support
  active: { bg: "bg-green-100 dark:bg-green-500/15", text: "text-green-700 dark:text-green-300", label: "Activo" },
  inactive: { bg: "bg-muted", text: "text-muted-foreground", label: "Inactivo" },
  blocked: { bg: "bg-red-100 dark:bg-red-500/15", text: "text-red-700 dark:text-red-300", label: "Bloqueado" },
};
    ...
    <div className="flex flex-col h-full bg-background">
          ...
          <div className="max-w-5xl grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-card rounded-xl border border-border p-6 text-center">
                <div className={`relative h-20 w-20 mx-auto rounded-full flex items-center justify-center text-3xl font-semibold text-muted-foreground ${client.hasAdTracking ? "ring-3 ring-blue-500 ring-offset-2 bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-blue-500/20 dark:to-indigo-500/20" : "bg-muted"}`}>
                  ...
                  {client.hasAdTracking && (client.adLastPlatform || client.adFirstPlatform) && (
                    <span className="absolute -bottom-0.5 -right-0.5 h-5 w-5 rounded-full bg-card border border-border flex items-center justify-center shadow-sm">
                ...
                <h2 className="text-lg font-semibold text-foreground mt-3">{fullName}</h2>
                <p className="text-sm text-muted-foreground mt-0.5">{client.email || "Sin email"}</p>
              ...
              <div className="bg-card rounded-xl border border-border p-5">
                <h3 className="text-sm font-semibold text-foreground mb-4">Conversaciones</h3>
                {conversationsLoading ? (
                  <div className="flex items-center justify-center py-6">
                    <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-emerald-600" />
                  </div>
                ) : conversations.length === 0 ? (
                  <p className="text-sm text-muted-foreground py-4 text-center">No hay conversaciones</p>
                ) : (
                  <div className="space-y-2">
                    {conversations.map((conv) => {
                      const channel = conv.inbox?.channel || "";
                      const channelInfo = CHANNEL_ICONS[channel] || { icon: MessageSquare, color: "text-muted-foreground", bg: "bg-muted" };
              ...
              <div className="bg-card rounded-xl border border-border p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 rounded-md text-xs text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 font-medium transition-colors"
                  >
                ...
                {notesLoading ? (
                  ...
                  <div className="text-center py-6">
                    ...
                    <button
                      onClick={() => setShowNoteModal(true)}
                      className="text-xs text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 font-medium mt-1"
                    >
                  ...
                  <div className="space-y-3">
                    {notes.map((note) => {
                      ...
                      return (
                        <div key={note.id} className="group p-3 rounded-lg border border-border hover:bg-muted transition-colors">
                          <div className="flex items-start justify-between gap-2">
                            <div
                              className="text-xs text-foreground leading-relaxed flex-1 [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4 [&_a]:text-emerald-600 dark:[&_a]:text-emerald-400 [&_a]:underline [&_b]:font-semibold"
                            ...
                            {canDelete && (
                              <button
                                ...
                                className="h-5 w-5 rounded flex items-center justify-center text-muted-foreground hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all shrink-0"
                              >
              ...
              <div className="bg-card rounded-xl border border-border p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-brand-600 dark:text-brand-300 hover:bg-brand-50 dark:hover:bg-brand-700/40 rounded-lg transition-colors"
                  >
                ...
                {contactEventsLoading ? (
                  ...
                  <div className="text-center py-6">
                    ...
                    <p className="text-[11px] text-muted-foreground/70 mt-1">Registra hitos como compras, citas o demos</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {contactEvents.map((evt) => (
                      <div key={evt.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-muted group">
                        <div className="h-8 w-8 rounded-full bg-muted flex items-center justify-center shrink-0">
                          ...
                          {evt.type === 'custom' && <Zap className="h-4 w-4 text-muted-foreground" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            ...
                            {evt.dispatched && <span className="ml-1.5 text-green-600 dark:text-green-400">✓ Reportado a ads</span>}
                          </p>
                        </div>
                        {evt.value && (
                ...
                <InfoSection key={group.key} title={group.label}>
                  {group.fields.map((field) => {
                    ...
                    if (field.fieldKey === "tags") {
                      ...
                      return (
                        <div key={field.id} className="flex items-center justify-between py-2.5 border-b border-border last:border-0">
                          <span className="text-sm text-muted-foreground">{field.fieldLabel}</span>
                          <div className="flex flex-wrap gap-1 justify-end max-w-[60%]">
                            {client.tags.map((tag) => (
                              <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/25">
      ...
      {showEventForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setShowEventForm(false)}>
          <div className="w-full max-w-md bg-card text-card-foreground rounded-2xl shadow-2xl border border-border overflow-hidden" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            ...
            <div className="px-6 py-5 space-y-5">
                  ...
                  {([
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2.5 p-3 rounded-xl border text-left transition-all ${isSelected ? "border-brand-500 bg-brand-50/50 dark:bg-brand-700/30 ring-1 ring-brand-500/20" : "border-border hover:bg-muted"}`}
                      >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 text-sm border border-border bg-background text-foreground placeholder:text-muted-foreground rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-400"
                />
                <p className="text-[11px] text-muted-foreground mt-1">Describe brevemente qué pasó con este contacto</p>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="0"
                    className="flex-1 px-3.5 py-2.5 text-sm border border-border bg-background text-foreground placeholder:text-muted-foreground rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-400"
                  />
                  <select
                    value={eventForm.currency}
                    onChange={(e) => setEventForm({ ...eventForm, currency: e.target.value })}
                    className="w-24 px-3 py-2.5 text-sm border border-border bg-background text-foreground rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-400"
                  >
                ...
                <p className="text-[11px] text-muted-foreground mt-1">Si este evento tiene un valor de venta, se reportará a las plataformas de ads</p>
              </div>

              {/* Info box */}
              <div className="flex items-start gap-2.5 p-3 rounded-xl bg-blue-50 border border-blue-100 dark:bg-blue-500/10 dark:border-blue-500/25">
...
function ActionButton({ icon: Icon, label, onClick }: { icon: React.ElementType; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-1 group">
      <div className="h-10 w-10 rounded-full border border-border flex items-center justify-center group-hover:bg-muted transition-colors">
...
function TimelineItem({ activity }: { activity: ActivityRecord }) {
  const iconMap: Record<string, { icon: React.ElementType; bg: string; color: string }> = {
    status_changed: { icon: ArrowRightLeft, bg: "bg-blue-100 dark:bg-blue-500/15", color: "text-blue-600 dark:text-blue-400" },
    assigned: { icon: UserPlus, bg: "bg-indigo-100 dark:bg-indigo-500/15", color: "text-indigo-600 dark:text-indigo-400" },
    note_created: { icon: StickyNote, bg: "bg-amber-100 dark:bg-amber-500/15", color: "text-amber-600 dark:text-amber-400" },
    message_received: { icon: MessageSquare, bg: "bg-green-100 dark:bg-green-500/15", color: "text-green-600 dark:text-green-400" },
    message_sent: { icon: Send, bg: "bg-emerald-100 dark:bg-emerald-500/15", color: "text-emerald-600 dark:text-emerald-400" },
    contact_created: { icon: Plus, bg: "bg-muted", color: "text-muted-foreground" },
    contact_updated: { icon: Pencil, bg: "bg-muted", color: "text-muted-foreground" },
    tag_added: { icon: Tag, bg: "bg-purple-100 dark:bg-purple-500/15", color: "text-purple-600 dark:text-purple-400" },
    tag_removed: { icon: Tag, bg: "bg-red-100 dark:bg-red-500/15", color: "text-red-600 dark:text-red-400" },
    conversion_event: { icon: Zap, bg: "bg-amber-100 dark:bg-amber-500/15", color: "text-amber-600 dark:text-amber-400" },
  };
  const config = iconMap[activity.type] || { icon: Clock, bg: "bg-muted", color: "text-muted-foreground" };
  ...
  return (
    <div className="flex items-start gap-3 relative">
      ...
      <div className="flex-1 min-w-0 pt-0.5">
        <p className="text-xs text-foreground">{activity.description || activity.type}</p>
