...
import { Radio, Plus, Trash2, ShoppingCart, CalendarCheck, Star, UserPlus, ArrowRightLeft } from "lucide-react";
...
const PLATFORMS = [
  { value: "meta", label: "Meta (Facebook/Instagram)", fields: [
  ...
  { value: "google", label: "Google Ads", fields: [
    ...
    { key: "developerToken", label: "Developer Token", placeholder: "aBcDeFg...", type: "password" },
  ]},
  { value: "tiktok", label: "TikTok Ads", fields: [
...
const CONVERSION_MAP = [
  ...
  { type: "appointment", label: "Cita agendada", icon: CalendarCheck, meta: "Schedule", google: "schedule", tiktok: "Contact", value: false },
  { type: "qualified", label: "Lead calificado", icon: Star, meta: "Lead", google: "lead", tiktok: "SubmitForm", value: false },
  { type: "registration", label: "Registro", icon: UserPlus, meta: "CompleteRegistration", google: "signup", tiktok: "CompleteRegistration", value: false },
  { type: "subscription", label: "Suscripción", icon: ArrowRightLeft, meta: "Subscribe", google: "subscribe", tiktok: "Subscribe", value: true },
];
...
export function AdPlatformsCard() {
  ...
  const [newPlatform, setNewPlatform] = useState({ platform: "meta", credentials: {} as Record<string, string> });
  ...
  const handleAddPlatform = async () => {
    try {
      const { data } = await api.post("/conversions/platforms", { tenantId, platform: newPlatform.platform, credentials: newPlatform.credentials, isActive: true });
      setPlatforms((prev) => [...prev, data]);
      setShowAddPlatform(false);
      setNewPlatform({ platform: "meta", credentials: {} });
      ...
      <div className="p-5 space-y-6">
        ...
        <div>
          ...
          {showAddPlatform && (
            <div className="mt-3 p-4 rounded-xl border border-gray-200 bg-gray-50/50 space-y-3">
              <select value={newPlatform.platform} onChange={(e) => setNewPlatform({ platform: e.target.value, credentials: {} })} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500/30">
        ...
        {connectedPlatformNames.length > 0 && (
          <div className="border-t border-gray-100 pt-5">
            ...
            <p className="text-[11px] text-gray-400 mb-3">Cada evento se envía a las plataformas conectadas con el nombre correcto de cada una</p>
              ...
              {CONVERSION_MAP.map((evt) => {
                ...
                const activeNames = connectedPlatformNames
                  .map((p) => {
                    ...
                    if (p === "google") return `Google: ${evt.google}`;
