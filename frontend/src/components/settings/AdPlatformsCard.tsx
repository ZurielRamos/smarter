...
export function AdPlatformsCard() {
  ...
  useEffect(() => {
    ...
    if (selectAccount && accounts) {
      setGooglePlatformId(selectAccount);
      setGoogleAccounts(accounts.split(","));
    }
  ...
  const handleSelectGoogleAccount = async (customerId: string) => {
    if (!googlePlatformId) return;
    try {
      await api.get("/conversions/google/select-account", { params: { platformId: googlePlatformId, customerId } });
      toast.success("Google Ads conectado correctamente");
      setGoogleAccounts([]);
