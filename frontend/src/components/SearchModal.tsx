        ...
        >
          ...
          <div className="max-h-[50vh] overflow-y-auto">
            {!query.trim() && (
              <div className="px-5 py-8 text-center">
                <p className="text-sm text-muted-foreground">Escribe para buscar contactos o mensajes</p>
                <p className="text-xs text-muted-foreground mt-1">Mínimo 2 caracteres</p>
              </div>
            )}
            ...
            {hasResults && (
              <div className="py-2">
                ...
                {results.contacts.length > 0 && (
                  <div>
                    <p className="px-5 py-1.5 text-[10px] font-semibold text-muted-foreground uppercase">Contactos</p>
                    {results.contacts.map((contact) => {
                      ...
                      return (
                        ...
                        >
                          <div className="h-8 w-8 rounded-full bg-muted flex items-center justify-center text-xs font-semibold text-muted-foreground shrink-0 overflow-hidden">
                            {contact.avatarUrl ? (
                          ...
                          {contact.status && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground capitalize">{contact.status}</span>
                          )}
                ...
                {results.messages.length > 0 && (
                  <div>
                    <p className="px-5 py-1.5 text-[10px] font-semibold text-muted-foreground uppercase mt-1">Mensajes</p>
          ...
          <div className="px-5 py-2.5 border-t border-border flex items-center justify-between">
            <p className="text-[11px] text-muted-foreground">
              <kbd className="px-1 py-0.5 rounded bg-muted text-muted-foreground font-mono mr-1">↑↓</kbd> Navegar
              <kbd className="px-1 py-0.5 rounded bg-muted text-muted-foreground font-mono mx-1">↵</kbd> Abrir
            </p>
            <p className="text-[11px] text-muted-foreground">
              <kbd className="px-1 py-0.5 rounded bg-muted text-muted-foreground font-mono">⌘K</kbd> para buscar
            </p>
          </div>
        </motion.div>
      </motion.div>
