  ...
  return (
    <div ref={containerRef} className={cn("relative", className)}>
      <button
        ...
        className={cn(
          "flex items-center justify-between gap-2 w-full px-3 py-2 text-sm border rounded-lg bg-card whitespace-nowrap transition-colors",
      ...
      {open && (
        <div className="absolute z-50 top-full left-0 mt-1 min-w-[140px] bg-card border border-border rounded-lg shadow-lg overflow-hidden p-1">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
