...
function Stepper({ value, onChange, min = 0, max = 1000, suffix = "px" }: { value: number; onChange: (v: number) => void; min?: number; max?: number; suffix?: string }) {
  return (
    <div className="inline-flex items-center border border-border rounded-full overflow-hidden">
      <button onClick={() => onChange(Math.max(min, value - 10))} className="w-7 h-7 flex items-center justify-center text-muted-foreground hover:bg-muted text-xs">&minus;</button>
      <input
        type="number"
        value={value}
        onChange={(e) => onChange(Math.max(min, Math.min(max, parseInt(e.target.value) || 0)))}
        className="w-12 h-7 text-center text-[10px] font-semibold text-foreground border-x border-border focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
      />
      <button onClick={() => onChange(Math.min(max, value + 10))} className="w-7 h-7 flex items-center justify-center text-muted-foreground hover:bg-muted text-xs">+</button>
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      <div className="relative rounded-lg border border-border overflow-hidden bg-muted group">
        ...
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
          <button
            ...
            className="p-2 rounded-lg bg-card/90 text-foreground hover:bg-card shadow-sm transition-colors"
            title="Cambiar"
          >
          ...
          <button
            onClick={() => update("src", "")}
            className="p-2 rounded-lg bg-card/90 text-red-600 hover:bg-card shadow-sm transition-colors"
            title="Eliminar"
          >
      ...
      <div className="divide-y divide-border">
        ...
        <div className="py-3">
          ...
          <p className="text-[9px] text-muted-foreground mt-1">La imagen mantiene su proporcion</p>
        </div>
          ...
          <div className="flex gap-1">
            {["100%", "75%", "50%", "auto"].map((w) => (
              <button
                ...
                className={`flex-1 py-1.5 rounded-md text-[10px] font-medium border transition-colors ${p.width === w ? "bg-brand-50 border-brand-300 text-brand-700" : "border-border text-muted-foreground hover:bg-muted"}`}
              >
                {w}
              </button>
            ))}
