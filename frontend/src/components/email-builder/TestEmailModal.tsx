...
export function TestEmailModal({ open, onClose, subject, html }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      <div className="relative bg-card rounded-2xl shadow-2xl border border-border w-[440px] max-h-[80vh] overflow-hidden flex flex-col" style={{ animation: "slideIn 0.2s cubic-bezier(0.16, 1, 0.3, 1)" }}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div>
            <h3 className="text-sm font-semibold text-foreground">Enviar prueba</h3>
            <p className="text-[11px] text-muted-foreground mt-0.5">Envia un email de prueba con esta plantilla</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg text-muted-foreground hover:bg-muted hover:text-muted-foreground transition-colors">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto px-5 py-4 space-y-4">
          {/* Email to */}
          <div>
            <label className="block text-[10px] font-medium text-muted-foreground uppercase mb-1">Correo destinatario</label>
            <input
              type="email"
              value={to}
              onChange={(e) => setTo(e.target.value)}
              placeholder="ejemplo@correo.com"
              className="w-full px-3 py-2 rounded-lg border border-border text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
              autoFocus
            />
          </div>
          ...
          {/* From name */}
          <div>
            <label className="block text-[10px] font-medium text-muted-foreground uppercase mb-1">Nombre del remitente</label>
            <input
              type="text"
              value={fromName}
          ...
          {usedVariables.length > 0 && (
            <div>
              <label className="block text-[10px] font-medium text-muted-foreground uppercase mb-2">Variables de prueba</label>
              <div className="space-y-2">
                {usedVariables.map((v) => (
                  <div key={v.field} className="flex items-center gap-2">
                    <span className="text-[11px] text-muted-foreground w-24 shrink-0 truncate">{v.label}</span>
                    <input
                      type="text"
                      ...
                      className="flex-1 px-2.5 py-1.5 rounded-md border border-border text-xs focus:outline-none focus:ring-2 focus:ring-brand-500"
                    />
                  </div>
                ))}
              </div>
